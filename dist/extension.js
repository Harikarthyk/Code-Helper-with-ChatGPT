...
var util = __webpack_require__(4);
var Stream = (__webpack_require__(5).Stream);
var DelayedStream = __webpack_require__(6);
...
module.exports =
{
  parallel      : __webpack_require__(16),
  serial        : __webpack_require__(23),
  serialOrdered : __webpack_require__(24)
};
...
var iterate    = __webpack_require__(17)
  ...
  , terminator = __webpack_require__(22)
  ;
...
var abort = __webpack_require__(20)
...
var serialOrdered = __webpack_require__(24);
...
module.exports = function () {
  if (!debug) {
    try {
      ...
      debug = __webpack_require__(30)("follow-redirects");
    }
    catch (error) { /* */ }
...
function setup(env) {
	...
	createDebug.humanize = __webpack_require__(33);
...
const axios_1 = __webpack_require__(44);
// Some imports not used depending on template conditions
// @ts-ignore
const common_1 = __webpack_require__(75);
// @ts-ignore
const base_1 = __webpack_require__(76);
...
axios.VERSION = (__webpack_require__(68).version);
...
var mergeConfig = __webpack_require__(70);
var validator = __webpack_require__(71);
...
var transitionalDefaults = __webpack_require__(56);
...
const packageJson = __webpack_require__(78);
class Configuration {
    constructor(param = {}) {
        ...
        if (!this.formDataCtor) {
            this.formDataCtor = __webpack_require__(2);
        }
    }
    /**
...
const openai_1 = __webpack_require__(42);
...
const breakDownCode = async (progress, token, bearerToken, prompt) => {
    ...
    const response = await triggerAPI(responseOptions, bearerToken);
...
const addCommentsToMethod = async (progress, token, bearerToken, prompt) => {
    token.onCancellationRequested(() => {
        console.log("User canceled the long running operation");
    });
    progress.report({ increment: 0 });
...
const findBugsInCode = async (progress, token, bearerToken, prompt) => {
    token.onCancellationRequested(() => {
        console.log("User canceled the long running operation");
    });
    progress.report({ increment: 0 });
...
const getApiFromUser = async () => {
    ...
    if (!apiKey || apiKey === "YOUR_API_KEY") {
        ...
        if (_apiKey) {
            try {
                await config.update("apiKey", _apiKey, vscode.ConfigurationTarget.Global);
                token = _apiKey;
                USER_TOKEN = _apiKey;
...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codeHelper.breakdownCode", async (document, range) => {
        ...
        if (editor) {
            try {
                ...
                vscodeProgressIndicator("Processing your code with OpenAI ", breakDownCode, { token, prompt });
            }
            catch (error) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codeHelper.writeTestCase", async (document, range) => {
        ...
        if (editor) {
            try {
                ...
                const prompt = `${prefix} ${text}`;
                vscodeProgressIndicator("Writing test case with OpenAI ", writeTestCase, { token, prompt });
            }
            catch (error) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codeHelper.refactorWithOpenAI", async (document, range) => {
        ...
        if (editor) {
            try {
                ...
                const prompt = `${prefix} ${text}`;
                vscodeProgressIndicator("Refactoring your code with OpenAI ", refactorCode, { token, prompt });
            }
            catch (error) {
                console.log("Error in test case generation");
            }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codeHelper.addCommentsToMethod", async (document, range) => {
        ...
        if (editor) {
            try {
                ...
                vscodeProgressIndicator("Adding comments to your code with OpenAI ", addCommentsToMethod, { token, prompt });
            }
            catch (error) {
                console.log("Error in test case generation");
            }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codeHelper.getAnsForComment", async (document, range) => {
        ...
        if (editor) {
            try {
                ...
                const text = editor.document.getText(selection);
                // added a loader to show that the extension is working.
                vscodeProgressIndicator("Getting the answer for your comment with OpenAI ", getAnsForComment, { token, prompt: text });
            }
            catch (error) {
                vscode.window.showErrorMessage("Error while processing your code. Please try again.");
            }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("codeHelper.findBugs", async (document, range) => {
        let token = await getApiFromUser();
        if (!token) {
            return;
        }
        ...
        if (editor) {
            try {
                ...
                const text = editor.document.getText(selection);
                // added a loader to show that the extension is working.
                vscodeProgressIndicator("Finding the bugs in your code...", findBugsInCode, { token, prompt: text });
            }
            catch (error) {
                vscode.window.showErrorMessage("Error while processing your code. Please try again.");
            }
